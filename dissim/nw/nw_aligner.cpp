/* $Id: nw_aligner.cpp 506310 2016-07-06 15:57:31Z kiryutin $
 * ===========================================================================
 *
 ...
 * 
 * Slava Brover: removed NCBI C++ ToolKit, disabled threads, removed warnings 
 *
...
#include "ncbi_system.hpp"
#include "align_exception.hpp"
#include "nw_formatter.hpp"
#include "Seq_id.hpp"
#include "Dense_seg.hpp"
...
{
    if(!seq1 || !seq2) {
        NCBI_THROW(CAlgoAlignException, eBadParameter, g_msg_NullParameter);
    }
	...
	if(iErrPos1 < len1) {
          //CNcbiOstrstream oss;
	    throw runtime_error ("The first sequence is inconsistent with the current scoring matrix type. Position = " + to_string(iErrPos1) 
	...
	if(iErrPos2 < len2) {
          //CNcbiOstrstream oss;
	    throw runtime_error ("The second sequence is inconsistent with the current scoring matrix type. Position = " + to_string (iErrPos2)
...
const
{
    const unsigned char c1 = (unsigned char) m_Seq1[i1];
    const unsigned char c2 = (unsigned char) m_Seq2[i2];
...
{
    const int dim = (int) m_Transcript.size();
    string s;
    s.resize((size_t) dim);
    ...
    for (int k (dim - 1); k >= 0; --k) {
        ...
        ETranscriptSymbol c0 (m_Transcript[(size_t) k]);
        char c (0);
        switch ( c0 ) {
...
bool CNWAligner::x_CheckMemoryLimit()
{
    ...
    if(gdim) {
        ...
        mem = double(dim1) * (double) dim2 * (double) elem_size;
        if(mem <= (double) m_MaxMem) {
        ...
        mem = double(m_SeqLen1 + 1) * double (m_SeqLen2 + 1) * (double) elem_size;
    }

    return mem < (double) m_MaxMem;
}
                       ...
                       size_t start1, size_t start2) const
{
    ...
    if( IsSmithWaterman() ) {
        ...
        //cut trailing gaps
        for(size_t endi = dim-1; endi + 1 >= 1; --endi) {
    ...
    if(m_esf_L1) {
        ...
        for(size_t i1 = 0; i1 < dim; ++i1) {
            if(transcript[i1] == eTS_Insert) ++g; else break;
        }
        if(g > 0) {
            score -= (m_Wg + (int)g*m_Ws);
        }
    }
    ...
    if(m_esf_L2) {
        size_t g = 0;
        for(size_t i1 = 0; i1 < dim; ++i1) {
            if(transcript[i1] == eTS_Delete) ++g; else break;
        }
        if(g > 0) {
            score -= (m_Wg + (int)g*m_Ws);
        }
    ...
    const char* beg, const char* end, size_t& err_index)
{
    ...
    for(const char* p = beg; p < end; ++p) {
        ...
        default:  err_index = (size_t) (p - beg); return 0x40; // incorrect char
        }
        fp = 0x3F & ((fp << 2) | code);
    }
