/* $Id: nw_band_aligner.cpp 198851 2010-07-29 15:16:34Z kiryutin $
 * ===========================================================================
 *
...
{
    switch(where) {
    case 0: m_Shift = (long) offset;  break;
...
CNWAligner::TScore CBandAligner::x_Align(SAlignInOut* data)
{
    ...
    TScore V1 (wgleft2 + wsleft2 * (int) ibeg);

    const long d1 ((long) N2 + (long) m_band + m_Shift);
    const size_t iend (d1 <= 0? 0: (d1 < long(N1)? (size_t) d1: N1));
    ...
    for(size_t i (ibeg); i < iend && !m_terminate; ++i) {
        ...
        const long   d2   ((long) i - m_Shift - (long) m_band);
        const size_t jbeg (d2 < 0? 0: (size_t) d2);
        const long   d3   ((long) i - m_Shift + (long) m_band + 1);
        const size_t jend (d3 < long(N2)? (size_t) d3: N2);
        if(jend == N2) ++jendcnt;

        const Uint1 ci ((Uint1) seq1[i]);
        
        if(i == 0) {
            V2 = wgleft1 + wsleft1 * (TScore) jbeg;
            TScore s (wgleft1);
        ...
        const long jbeg0 ((long) i - m_Shift - (long) m_band);
        if(jbeg0 < 0) {
            k += jbeg + (size_t) (- jbeg0);
        }
        if(k - 1 > m_TermK && (k & 1)) {
        ...
        for(j = jbeg; j < jend; ++j) {            
            ...
            else if(j == 0 && i < (size_t) m_Shift + m_band) {
                V1 += wsleft2;
                E = V1 + wg1 + ws1;
                tracer = 0;
            }
...
bool CBandAligner::x_CheckMemoryLimit()
{
    ...
    if(gdim) {
        ...
        double mem = double(max(dim1, dim2))*(double)m_band*(double)elem_size;
        ...
        for(size_t i = 4; i < gdim; i += 4) {
            ...
            mem = double(max(dim1, dim2))*(double)m_band*(double)elem_size;
            if(mem >= (double)m_MaxMem) {
                return false;
            }
        }
        ...
        double mem = double(max_len)*(double)m_band*(double)elem_size;
