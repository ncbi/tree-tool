...
if [ $# -ne 7 ]; then
  ...
  echo "Output: #6.dm, #6.tree, #6.nw"
    # "; for #2 = 1 and #5 = 'min_edit': #6.gr, #6.roots"
  echo "#1: FASTA"
...
$THIS/../dissim/fasta2dissim  -threads $CORES  $FASTA  $PAR  -dataset $OUT


super_section "Building tree"
# PAR
$THIS/makeDistTree  -data $OUT  -dissim_attr "dissim"  -variance pow  -variance_power 5  -variance_dissim  -optimize  -subgraph_iter_max 5  -output_tree $OUT.tree  -threads $CORES
