...
if [ "$TARGET" ]; then
  setIntersect.sh $TMP.list1 $TARGET 0 > $TMP.list1-good 
  setIntersect.sh $TMP.list2 $TARGET 0 > $TMP.list2-good 
else
  cp $TMP.list1 $TMP.list1-good
  cp $TMP.list2 $TMP.list2-good
fi

N=`setIntersect.sh $TMP.list1-good $TMP.list2-good 0 | wc -l`
if [ $N -le 2 ]; then
  echo "Intersection = $N"
  exit 0
fi
...
setMinus $TMP.list1 $TMP.list2-good > $TMP.list1-del
setMinus $TMP.list2 $TMP.list1-good > $TMP.list2-del
