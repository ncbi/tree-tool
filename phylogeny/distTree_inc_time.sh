...
set Leaves = `grep '^# Leaves: ' $1/hist/makeDistTree.$2`
if ($?) exit 1

set Dissim = `grep '^# Dissimilarities: ' $1/hist/makeDistTree.$2`
if ($?) exit 1

set CHRON_topology = `grep '^CHRON: Initial topology: ' $1/hist/makeDistTree.$2`
if ($?) exit 1

set CHRON_Leaves = `grep '^CHRON: Optimizing new leaves: ' $1/hist/makeDistTree.$2`
if ($?) exit 1
...
if ($3) then
  set L = `grep '^CHRON: getBestChange: ' $1/hist/makeDistTree.$2`
  if ($?) exit 1
  set CHRON_getBestChange = $L[3]
  
  set L = `grep '^CHRON: tree2subgraph: ' $1/hist/makeDistTree.$2`
  if ($?) exit 1
  set CHRON_tree2subgraph = $L[3]
  
  set L = `grep '^CHRON: tree2subgraphDissim: ' $1/hist/makeDistTree.$2`
  if ($?) exit 1
  set CHRON_tree2subgraphDissim = $L[3]
  
  set L = `grep '^CHRON: subgraphOptimize: ' $1/hist/makeDistTree.$2`
  if ($?) exit 1
  set CHRON_subgraph = $L[3]
  
  set L = `grep '^CHRON: subgraph2tree: ' $1/hist/makeDistTree.$2`
  if ($?) exit 1
  set CHRON_subgraph2tree = $L[3]
endif


set Radius = `grep '^Ave. radius: ' $1/hist/makeDistTree.$2`
if ($?) exit 1
