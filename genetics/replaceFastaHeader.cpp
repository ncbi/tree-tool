...
struct ThisApplication : Application
{
	...
	void body () const final
  {
	  ...
	  const string headersFName = getArg ("headers");
    ...
    map<string,string> id2header;
    {
      ...
      while (f. nextLine ())
      {
        ...
        f. line. erase (0, name. size ());
        trim (f. line);
        id2header [ name] = f. line;
      }
    }
	  ...
	  while (faIn. next ())
	  {
	    ...
	    if (const string* s = findPtr (id2header, seq->getId ()))
	      seq->name = *s;
